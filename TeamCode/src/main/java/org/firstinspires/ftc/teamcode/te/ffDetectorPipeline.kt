...
class ffDetectorPipeline(
    ...
    companion object Config {
        ...
        var HORIZON: Int = ((160.0 / 320.0) * CAMERA_WIDTH).toInt()
        ...
        try { // try catch in order for opMode to not crash and force a restart
            ...
            barcode = if (maxWidth >= MIN_WIDTH) {
                ...
                if (aspectRatio > BOUND_RATIO)
                    Barcode.RIGHT // height variable is now FOUR
                else
                    Barcode.CENTER // height variable is now ONE
            }
            else {
                Barcode.LEFT // height variable is now ZERO
            }

            if (debug) telemetry?.addData("Vision: Height", barcode)
